...
class ACO {
    ...
    init(start: GridPoint, sightsForVisit: [Sight], grid: [[CellType]]) {
        points = [start] + sightsForVisit.map(\.entryCord)

        size = points.count
        self.grid = grid
        pheromones = Array(repeating: Array(repeating: 1.0, count: size), count: size)
        distances = Array(repeating: Array(repeating: 0, count: size), count: size)
    ...
    func optimalPath() -> [GridPoint] {
        if points.isEmpty {
            return []
        }
        var path = AntPath(dist: Int.max)
        for _ in 1 ... 100 {
            ants.removeAll()
            createAnts()
